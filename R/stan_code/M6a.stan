...
parameters {
  real global_alpha;  // Population-level intercept
  vector[6] global_beta;  // Population-level slopes
  
  real mu_alpha;  // Group-level intercept
  vector[6] mu_beta;  // Group-level slopes
  
  real<lower=0> sigma_alpha;  // Group-level standard deviation for intercept
  vector<lower=0>[6] sigma_beta;  // Group-level standard deviation for slopes
  
  real<lower=0, upper=1> gamma;  // Decay parameter
  
  vector[S] alpha_subject_raw;  // Subject-level intercepts (non-centered)
  matrix[6, S] beta_subject_raw;  // Subject-level slopes (non-centered)
  
  vector[E] alpha_session_raw;  // Session-level intercepts (non-centered)
  matrix[6, E] beta_session_raw;  // Session-level slopes (non-centered)
}

transformed parameters {
  vector[S] alpha_subject = global_alpha + mu_alpha + alpha_subject_raw * sigma_alpha;
  matrix[6, S] beta_subject;
  
  vector[E] alpha_session = global_alpha + mu_alpha + alpha_session_raw * sigma_alpha;
  matrix[6, E] beta_session;
  
  for (j in 1:6) {
    beta_subject[j] = global_beta[j] + mu_beta[j] + beta_subject_raw[j] * sigma_beta[j];
    beta_session[j] = global_beta[j] + mu_beta[j] + beta_session_raw[j] * sigma_beta[j];
  }
}

model {
  // Priors for population-level effects
  global_alpha ~ normal(0, 10);
  global_beta ~ normal(0, 10);
